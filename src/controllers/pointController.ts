...
import { addHistoryPoint, createOrUpdatePoint, deletePoints, getAllPointUser, getHistoryPoint, getPointById } from "../services/point.service";
...
export const getAllHistoryPoint = async (req: Request, res: Response) => {
  try {
...
export const addHistoryPointController = async (req: Request, res: Response) => {
  try {
    const payload = req.body
    const history = await addHistoryPoint(payload)
    return res.status(201).json({
      message: "Created History Points Successfully!",
      data: history
    })
  } catch (error: any) {
    console.error("Error in addHistoryPointController", error)
    return res.status(500).json({ message: error.message })
  }
}
